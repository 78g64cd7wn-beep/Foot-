#!/usr/bin/env python3
"""
Agent IA - Actu foot en temps réel (étape 1/2).
Vérifie s'il y a une actualité football notable, génère le texte ET une image,
puis écrit pending.json pour que l'étape de publication prenne le relais.
"""
...
if not API_KEY:
    print("Erreur : ANTHROPIC_API_KEY n'est pas définie.")
    sys.exit(1)
...
SYSTEM_PROMPT = f"""Tu es un agent d'actualité football, spécialisé dans le football français et européen.

Ta mission à chaque exécution :
1. Utilise la recherche web pour vérifier s'il y a une actualité football NOTABLE et RÉCENTE (moins de quelques heures) : transfert officialisé, résultat de match important, blessure d'un joueur majeur, déclaration marquante, record battu.
2. Compare avec la liste des sujets déjà traités. Ne répète jamais un sujet déjà couvert.
3. Sois strict : une rumeur non confirmée ou un simple entraînement ne suffisent pas.

DEUX CAS POSSIBLES :

CAS A — actu nouvelle et notable trouvée :
Réponds EXACTEMENT dans ce format (une ligne par champ, rien d'autre) :
TITLE: [4 à 8 mots, percutant, pour affichage sur une image]
...
TOPIC_ID: [3-6 mots résumant le sujet, pour éviter les doublons]

CAS B — rien de notable :
Réponds UNIQUEMENT avec : {SKIP_MARKER}
"""
...
if not fields["CAPTION"] or not fields["TOPIC_ID"]:
    print("Réponse mal formée, on ignore ce cycle.")
    sys.exit(0)
...
print(f"Post préparé : {fields['TOPIC_ID']}")
